...
func init() {
	...
	recursivelyIgnoredPkgs = append(recursivelyIgnoredPkgs, "github.com/sudoless/zerolog")
	args := flag.Args()
	if len(args) != 1 {
...
func main() {
	...
	// get the github.com/sudoless/zerolog.Event type
	event := getEvent(p)
	if event == nil {
		fmt.Fprintln(os.Stderr, "Error: github.com/sudoless/zerolog.Event declaration not found, maybe zerolog is not imported in the scanned package?")
		os.Exit(1)
	}

	// get all selections (function calls) with the github.com/sudoless/zerolog.Event (or pointer) receiver
...
func getEvent(p *loader.Program) types.Type {
	for _, pkg := range p.AllPackages {
		if strings.HasSuffix(pkg.Pkg.Path(), "github.com/sudoless/zerolog") {
